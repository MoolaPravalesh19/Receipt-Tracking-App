...
const saveToDatabaseTool = createTool({
    ...
    handler: async ({ params, context }) => {
            ...
            async () => {
                try {
                    const { userId } = await convex.mutation(
                        api.receipts.updateReiptWithExtractedData,
                        {
        ...
        if (result?.addedToDb === "Success") {
            ...
            context.network?.kv.set("receipt", receiptId);
        }
        return result;
    }
...
export const databaseAgent = createAgent({
    name: 'Database Agent',
    description:'responsible for taking key information regarding receipts and saving it to the covex database.',
